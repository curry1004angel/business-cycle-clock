...
- 대부분 **1998년~현재**, 일부(엠파이어스테이트)는 2001년~.
- **PMI 메모**: ISM/S&P PMI는 무료로 "장기+최신+자동"을 동시에 주는 소스가 없어,
  더 빨리 발표되는 지역 연준 서베이로 대체했다. 실제 ISM/S&P 헤드라인을 보고 싶으면
  `data/pmi_manual.csv` 에 `date,pmi` 형식으로 월 1회 추가하면 합성에 함께 반영된다.
