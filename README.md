...
| 그룹 | 지표 | 소스 |
|---|---|---|
| 선행 | S&P500, 소비자심리(UMCSENT), 장단기금리차(T10Y2Y), 제조업 PMI(ISM) | Yahoo, FRED, 자동수집 |
| 동행 | 산업생산(INDPRO), 소매판매(RSAFS), 설비가동률(TCU), 수출(BOPTEXP) | FRED |
| 후행 | 실질GDP(GDPC1), 실업률(UNRATE), 임금(AHETPI), 주당노동시간(AWHMAN) | FRED |

- **1975년~현재.** 소매판매·수출은 1992~이며 없는 기간엔 합성에서 자동
  제외된다(1992년 이전은 판정 정밀도 다소 낮음).
- **PMI 메모**: 실제 ISM PMI를 쓴다.
  ...
  실측 대조 검증)로 채워져 있다. 2015-07 이후는 investing.com 발표 이력으로
  메웠고, **신규 발표분은 `scripts/update_pmi.py`가 매월 자동 수집**한다
  (1순위 네이버 모바일 검색 표, 폴백 investing.com — 후자는 2026-08부터
  GitHub Actions IP에 403). 전부 실패하면 앱의 수동 입력 폼으로 보완.
